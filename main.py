...
if __name__ == '__main__':
    ...
    # gpu_mq = MQ(GPU_HOST, GPU_PORT, GPU_USER, GPU_PASSWORD, GPU_QUEUE, GPU_ROUTING_KEY, GPU_EXCHANGE, GPU_R_EXCHANGE,GPU_R_QUEUE)
    # gpu_info = GPU_Info(gpu_mq)
    # gpu_info.start()
    # Test - send
    # msg = json.dumps(
    #     {"TIME": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "TYPE": "Str/Json", "DATA": "First msg From AI"})
    # ai_mq.send(msg)
    # Run
    try:
        ai_mq.rece()
